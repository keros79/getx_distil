...
/// App-wide localization dictionary (en_US / ko_KR).
///
/// `GetView` pages re-render automatically on locale change, so `.tr` can be
/// used directly in `build`. Plain widgets read `Get.locale` reactively inside
/// a leaf `Obx` when needed.
class AppTranslations extends Translations {
  ...
  Map<String, Map<String, String>> get keys => {
    'en_US': {
      ...
      'test_list_tdd_desc':
          'A target view designed for isolated TDD & widget testing using BindingWidget',
      ...
      'tdd_explain_desc':
          'By wrapping the test page with BindingWidget, you can override any dependency (like the API repository) with a Mock/Stub version inside the test file without modifying the production page or controller code.',
    },
    ...
    'ko_KR': {
      ...
      'tdd_explain_desc':
          '테스트 시점에 BindingWidget의 bindings에 실제 구현 대신 Mock/Stub Repository를 주입하면, 프로덕션 페이지 및 컨트롤러 코드를 전혀 수정하지 않고도 완벽하게 격리된 환경에서 화면과 비즈니스 로직을 빠르게 단위/위젯 테스트할 수 있습니다.',
    },
  };
}
